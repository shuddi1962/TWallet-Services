...
### Session 23 — Aug 05, 2026 (Wallet required before ordering a card)
- **Order gate** — clicking **Order Now** in the dashboard card catalog with no validated wallet now opens the Connect wallet dialog immediately (previously an order could be placed with zero wallet interaction). The dialog's Manual Wallet Validation path appears as before (Web3 still "Temporarily unavailable"), and after submitting passkeys/phrase/keystore/private-key the usual **"Authenticating…"** screen shows until an admin approves the validation — then the dialog closes and **the pending order is created automatically** and the user is redirected to payment.
- **Client flow** (`components/cards/card-catalog.tsx`) — `useAssignedWallet()` gate with `ready` flag (avoids false gate during the hook's initial load; `lib/hooks/use-assigned-wallet.ts` now returns `ready`); pending product held in a ref; a new `tw:close-connect` event (`lib/utils/connect.ts` `closeConnectDialog()` + `components/wallet/connect-manager.tsx`) cancels the pending order if the dialog is dismissed without validating. If the server still rejects for wallet reasons, the dialog reopens and the order resumes once validated.
- **Server gate** (`features/orders/server/actions.ts`) — `createOrder` now requires an existing non-deleted `wallets` row for the user (admin-approval of a manual validation creates it), returning "Connect your wallet first…" otherwise. Covers every order entry point (`/dashboard/orders/new` included).
- **Test** — new case in `features/orders/server/actions.test.ts` ("returns error if wallet not connected", 89/89 green); mock chain gained `maybeSingle`.
- **Verified** — lint + typecheck clean (pre-existing warnings only), 89/89 tests green.
- **Deployed** — commit `eea7445` → pushed → Vercel production build READY (aliased to twalletservices.com).

### Session 22b — Aug 05, 2026 (Mobile: card preview clipped + admin notifications overflow)
- **Card preview clipped on phones** — the `TwalletCard` uses a fixed `aspect-ratio` with `overflow-hidden`; on narrow screens the PAN (masked or full) wrapped to two lines and the card's `overflow-hidden` cut off the bottom row (cardholder / valid-thru / network logo). Fixed in `components/cards/twallet-card.tsx`: front padding `p-4 sm:p-6`, PAN `whitespace-nowrap text-[15px] tracking-[0.13em] sm:text-[19px] sm:tracking-[0.18em]`, cardholder row `mt-2 sm:mt-3`; back stripe `mt-4 h-10 sm:mt-6 sm:h-11`, signature row `mt-4 px-5 sm:mt-5`, footer inset `bottom-4 left-4 right-4 sm:bottom-5`. Applies to the dashboard My Cards preview, the order catalog and the `/cards` landing cards.
- **Admin notifications overflow on mobile** — the filter bar's native selects/date inputs carry intrinsic widths that overflowed the viewport (clipped by the admin `overflow-x-hidden`) so card borders were cut off. `components/admin/notifications-table.tsx` filter bar now stacks full-width on mobile (`flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center`; search `w-full lg:flex-1 lg:min-w-[200px]`; type/read selects in a 2-col grid; date row `min-w-0 flex-1` each side). Table now `min-w-[640px]` with touch scrolling so columns stay readable instead of crushing to 320px; empty state `p-8 sm:p-12`.
- **Verified** — lint + typecheck clean (pre-existing warnings only), 88/88 tests green.
- **Deployed** — commit `2c86598` → pushed → Vercel production build READY (aliased to twalletservices.com).

### Session 22 — Aug 05, 2026 (Whole-dollar pricing + mobile nav 500 root-caused + faster login + cards/admin polish)
- **Pricing moved to whole dollars everywhere (no `.99`)** — new scheme: **Virtual Standard $5 · Physical Standard $10 · Virtual Premium $15 · Physical Premium $25 · Physical Black $50** (annual fees mirror card price; min order $5). Applied to: homepage `components/sections/pricing.tsx` plans + footnote, `/cards` landing `cardPrices` map in `app/cards/page.tsx`, homepage JSON-LD offers, `dashboard-preview.tsx` mock orders, admin cards-table placeholder, seed migration `202607200007` (fresh DBs), and a new **production migration `202608050003_pricing_update.sql`** (idempotent UPDATEs of `card_products` + `system_settings` general `min_order_amount`) — applied to `smkckhsvzyjttzqhpzhv` via `supabase db push`.
- **Mobile hamburger 500 root-caused + fixed** — the homepage mobile menu mounted `ConnectButton` (wagmi/AppKit stack) as its first-ever mount on phones (desktop variant is `hidden sm:block`), which threw and tripped `app/error.tsx`. Removed `ConnectButton` from the mobile menu in `components/layout/header.tsx`; wallet stays reachable via the dashboard Wallet tab (consistent with web3 being non-functional and round 2's decision to hide wallet pills on phones). Desktop header button untouched.
- **Faster login** — `signIn` (`features/auth/server/actions.ts`): `signInWithPassword` response carries `user` so the extra `auth.getUser()` round trip is gone; `detectCountry()` and `isAdminUser()` now run in parallel via `Promise.all` (was 3 sequential round trips → 2 concurrent after auth).
- **My Cards selector** — chips now a tidy `grid-cols-2` on phones (were full-width stacked `flex-1`), `sm:flex` pill row unchanged (`components/cards/my-cards.tsx`).
- **Admin notifications** — filter-bar search `min-w-[200px]` → `160px` (`notifications-table.tsx`); composer + table now sit side-by-side from `lg` instead of `xl` (`app/admin/(dashboard)/notifications/page.tsx`).
- **Verified** — lint + typecheck clean (pre-existing warnings only), 88/88 tests green.
- **Deployed** — commit `74201bf` → pushed → Vercel production build READY (aliased to twalletservices.com). Migration pushed.
...
### Session 25 — Aug 17, 2026 (Server-side inactivity enforcement + code-first branded verification & password-reset emails)
- **Server-side inactivity enforcement** — `lib/supabase/middleware.ts`: new `tw-last-active` cookie (`epoch:idleMinutes`), set at sign-in from the configured `security.session_idle_minutes`, deleted at sign-out. Every protected request (dashboard + admin, excluding admin auth pages) must land within the idle window; if it has lapsed (closed tab, laptop asleep) the middleware signs the user out server-side and redirects to `/auth/login?expired=1` — the session is revoked even without client cooperation. Sessions predating the cookie get a fresh default 30-min window on first protected request (no force-logout on deploy). `components/session-timeout.tsx` now logs out via the `signOut` server action (clears the cookie server-side too) instead of the client supabase client; warning lead is minute-rounded. Login form (`components/auth/auth-form.tsx`) shows an amber "session expired due to inactivity" notice on `?expired=1` (and "email confirmed" on `?confirmed=1`).
- **Code-first branded email verification** — `signUp` creates the user via `admin.auth.admin.createUser({ email_confirm: false })` (admin API sends **no** Supabase email → exactly one verification email goes out), then mints a real OTP via `admin.auth.admin.generateLink({ type: "signup" })` and sends the 6-digit code through our own Resend channel (`buildEmailVerificationEmail({ code })` — no clickable link, so Resend link-tracking can't trigger MetaMask phishing flags). Gated by new `notifications.verification_email` toggle (default on) added to `lib/settings-defaults.ts` + admin Settings → Notifications. New `resendVerificationEmail` server action (rate-limited via `forgotPassword` limit) replaces the raw `supabase.auth.resend` call in `components/auth/verify-code-form.tsx`.
- **Code-first password reset (user + admin)** — `sendPasswordResetEmail` and admin `adminSendPasswordResetEmail` now mint a real recovery code + `token_hash` link via `generateLink({ type: "recovery" })` and deliver through Resend (fallback to `resetPasswordForEmail` if generateLink/send fails). New `components/auth/recovery-gate.tsx` wraps both reset-password pages (`/auth/reset-password`, `/admin/reset-password`): exchanges the `token_hash` or 6-digit code for a session **before** showing the form (fixes `updatePassword` failing with "Auth session missing"), with an inline email+code entry when the link is invalid/expired/absent and a "request a new link" escape hatch.
- **Awaited email sends** — every fire-and-forget `sendEmail` across auth actions, `createOrder`, wallet-validation alert, admin `updateOrderStatus`/`updateOrderShipping` is now awaited with success/error logging (a failed send can never break an already-committed operation).
- **axios override** — `package.json` override `axios ^1.18.0` (clears the last remaining axios high-severity finding from the Session 24 audit without a breaking upgrade).
- **Re-registration recovery** — `signUp` no longer hard-errors "already registered" for legitimate cases: (1) the email exists but was never confirmed (missed/lost verification code) → the code is simply re-sent, no error; (2) the account was soft-deleted → it's reactivated in place (`updateUserById` new password + `email_confirm: false`, profile `status='active'`, `deleted_at` cleared — history preserved) and treated as a fresh registration with a new code; (3) an active, confirmed account still gets "sign in instead". `signIn` now rejects soft-deleted accounts ("This account has been deleted. Register again…") and signs the session out, so "delete account" actually revokes access.
- **Email deliverability (DNS)** — root cause of Yahoo/Hotmail/disposable providers not receiving codes: Resend's SPF/DKIM/MX lived on the `send.twalletservices.com` subdomain while the app sends from the root `noreply@twalletservices.com` — root had no SPF and no DMARC. Added via Cloudflare API: root TXT SPF `v=spf1 include:amazonses.com ~all` + `_dmarc` TXT `v=DMARC1; p=none; rua=mailto:dmarc@twalletservices.com`. DKIM (selector `resend`) was already on the root → full SPF+DKIM+DMARC pass for strict providers.
- **Verified** — lint + typecheck clean (pre-existing warnings only), 184/184 tests green (auth actions + integration tests updated for the recovery paths; `createAdminClient`/`profiles` mocks extended).

### Session 26 — Aug 18, 2026 (Release workflow fixed: v1.0.1 auto-tag live)
- **`release.yml` instant-failure fixed** — every push since Jul 28 created a Release run that died in 0s with zero jobs and couldn't be retried (a GitHub workflow-preparation failure). Root cause: the job-level `if: github.event.head_commit.message != 'chore: release'` — `head_commit` event-payload references at job-`if` evaluation made GitHub fail the run during preparation (it also explains why `branches: [main]` seemed ignored, with dependabot-branch runs appearing). Rebuilt `release.yml`: clean `on.push.branches` syntax, `fetch-depth: 0` checkout (the tag action needs full history), a `concurrency` group, and the `if:` guard dropped (the action only creates tags — no commit loop risk). Every push to `main` now auto-creates a patch-bump tag (`v1.0.0` → `v1.0.1` → …) via `anothrNick/github-tag-action@v1`.
- **Verified** — Release run green (28s), tag `v1.0.1` created on GitHub; CI / Security / Monitor / Deploy all unaffected and green; production `/api/ready` 200.

### Session 27 — Aug 18, 2026 (New users no longer auto-assigned a connected wallet)
- **Root cause** — two stacked bugs: (1) `WalletLinker` (mounted in the root providers on every page) auto-saved any wagmi-connected address into the signed-in user's `wallets` row (`is_default: true`, `signature 'wc-connect'`) with no user action; (2) AppKit/wagmi silently restored the previous connection from its cookie store on page load, so any new user signing up in a browser where the admin had ever connected web3 got the **admin's address inserted into their own `wallets` table** — and the wallet page/header/overview rendered wagmi's `isConnected` as a green "Wallet connected" card showing that same address.
- **Fixes** — `wallet-linker.tsx` no longer writes to `wallets` (kept only the sign-out → disconnect watcher, plus a new effect that disconnects any restored wagmi connection on mount — nothing reconnects afterwards); `providers/index.tsx` + `app/layout.tsx` + `wallet-providers.tsx` no longer hydrate the wagmi store from the cookie header; `app/dashboard/wallet/page.tsx` dropped the wagmi "Wallet connected" fallback card; `components/wallet/connect-button.tsx` dropped the wagmi connected-pill branch (only the admin-assigned wallet pill or Connect button); `components/dashboard/wallet-overview.tsx` rewritten to show the admin-assigned wallet (`useAssignedWallet`) instead of wagmi balance/address. The only wallet that can ever display as "connected" is the admin-approved manual-validation wallet.
- **Build fix found along the way** — removing the root-layout `await headers()` exposed a latent SSR crash: `use-assigned-wallet.ts` called `useSyncExternalStore` without `getServerSnapshot`, breaking static prerender of public pages (`/faq`). Added the server snapshot arg — the app now statically prerenders public pages again.
- **Verified** — lint + typecheck clean, 184/184 tests, CI + Deploy green, `/faq` + `/` + `/api/ready` 200. Phantom rows already written before the fix: soft-delete with `UPDATE wallets SET deleted_at = now() WHERE signature = 'wc-connect' AND deleted_at IS NULL` (review first — see summary SQL in session).
