...
- **Manual follow-up (recommended)** — in Supabase Auth → URL Configuration, set "Email confirmation URL" to `{{ .SiteURL }}/auth/confirm?token={{ .Token }}&type={{ .TokenType }}&email={{ .Email }}` so the link carries the email and verifies on first click without typing; optionally remove the clickable link from the confirmation template (code-first CTA) to dodge the tracking-URL phishing flag entirely.
