...
export async function verifyPayment(
  ...
  const decimals = input.tokenDecimals ?? chain.nativeDecimals;
  const expectedRaw = BigInt(
    Math.floor(Number(input.expectedAmount) * 10 ** decimals),
  ).toString();
  ...
  if (input.tokenAddress) {
    ...
    if (actualAmount !== expectedRaw) throw new Error("WRONG_AMOUNT");
  } else {
