...
export function Wallets() {
  return (
    <section id="wallets" className="relative py-20 lg:py-28">
      <Container>
        <FadeIn>
        ...
        <StaggerChildren className="mt-16 grid grid-cols-2 gap-4 sm:grid-cols-4">
          {wallets.map((wallet) => (
            <StaggerItem key={wallet.name}>
              <div className="group flex flex-col items-center rounded-2xl border border-white/5 bg-surface-900/30 p-8 transition-all hover:border-brand-500/20 hover:bg-surface-900/60 hover:shadow-lg hover:shadow-brand-500/5">
                <div
                  className="mb-4 flex h-14 w-14 items-center justify-center rounded-2xl text-xl font-bold text-white shadow-lg transition-transform group-hover:scale-110"
